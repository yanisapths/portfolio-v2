...
const TableOfContentsComponent: FC<Props> = () => {
    ...
    useEffect(() => {
        ...
        sections.forEach((section: Section, index: number) => {
            if (index === 0 ) {
                //Active logic for first section
                section.isActive = sections[index + 1].boundingTop > offsetY + marginTop
            } else {
                //lofic for middle section, in between
                if (sections[index + 1]){
                    section.isActive = sections[index + 1].boundingTop > offsetY + marginTop
                ...
                return (
                    <TimelineItem key={index}>
                        ...
                        <TimelineContent>
                            ...
                            style={{ textDecoration: 'none', color: section.isActive ? 
                                        'secondary' : 'primary' , textTransform: 'capitalize' , cursor: 'pointer' 
                            }}>
                                {section.topic}
                            </span>
                        </TimelineContent>
